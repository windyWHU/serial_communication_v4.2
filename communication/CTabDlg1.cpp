...
LRESULT CTabDlg1::OnDisplayRawData(WPARAM wParam, LPARAM lParam) {
    ...
    case 2: // UTF-8
    {
        // 推荐用 MultiByteToWideChar 明确指定长度
        int wlen = MultiByteToWideChar(CP_UTF8, 0, data.data(), (int)data.size(), NULL, 0);
        CStringW wstr;
        if (wlen > 0) {
            wstr.GetBufferSetLength(wlen);
            MultiByteToWideChar(CP_UTF8, 0, data.data(), (int)data.size(), wstr.GetBuffer(), wlen);
